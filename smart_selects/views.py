...
def is_m2m(model_class, field):
    ...
    if is_pre_19_syntax:
        try:
            if isinstance(
                getattr(model_class, field), ReverseManyRelatedObjectsDescriptor
            ):
                return True
        ...
        try:
            if (
                isinstance(getattr(model_class, field), ManyToManyDescriptor)
                and not getattr(model_class, field).reverse
            ):
                return True
        except AttributeError:
            return False
